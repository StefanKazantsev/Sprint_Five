...
def test_register_positive(chrome_browser,random_email,ok_passwd,name_user_for_register):
    ...
    chrome_browser.get(AllUrls.page_register)

    chrome_browser.find_element(By.XPATH, AllLocators.page_register_input_name).send_keys(name_user_for_register)
    chrome_browser.find_element(By.XPATH, AllLocators.page_register_input_email).send_keys(random_email)
